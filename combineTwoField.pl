...
# module: combine.pl
# This is to merge all serotype tables in TWORESULTS directory
...
foreach my $csv ( sort @tmp ) {
	...
	if ( $csv =~ /(\w+)_(TwoField_Serotype_Table_IMGT_HLA_\d+\.\d+\.\d+)_/ ) {
		$gene = $gene . "_" . $1;
		$db = $2;
	}
...
open( FILE, ">TWORESULTS/combined" . $gene . "_" . $db . "_" . $date . ".csv" );
print FILE $header . "\n";
