...
my $residues_all_ref = HLAC_INFO::RESIDUES("ALL");
...
my $known_cross_ref = HLAC_INFO::KNOWN_CROSS();
...
for ( my $index = 0; $index < scalar @group; $index++ ) {
	...
	my $residues_ref = HLAC_INFO::RESIDUES( $group[ $index ] );
