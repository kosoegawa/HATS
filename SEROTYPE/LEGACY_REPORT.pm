...
sub COMBINED {
	...
	foreach my $allele ( @$alleles_sorted_ref ) {		#go through all alleles
		...
		elsif ( exists $short_ref->{ $allele } ) {	# INCOMPLETE
			print FILE $allele . ",";
...
sub COMBINED_TWO {
	...
	foreach my $allele ( @$alleles_sorted_ref ) {		#go through all alleles
		...
		if ( exists $twoField{$twoField} ) {
			...
			if (( $serotype eq "A0305" ) && ( $allele =~ /A\*11/ )) {	# request from MFV on February 10 2021
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",A11,A11";
			}
			elsif ( $serotype eq "A0203" ) {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",A203,A2";
			}
			elsif ( $serotype eq "A0210" ) {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",A210,A2";
			}
			elsif ( $serotype eq "B0703" ) {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",B703,B7";
			}
			elsif ( $serotype eq "DR0103" ) {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",DR103,DR1";
			}
			elsif (( $serotype eq "A2403" ) || ( $serotype eq "A2410" ) || ( $serotype eq "A2423" )) {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",A2403,A9";
			}
			elsif (( $serotype eq "B4406" ) && ( $allele =~ /B\*51/ )) {	# request from MFV on February 10 2021
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . ",B51,B5";
			}
			else {
				print FILE $twoField . "," . $serotype . "," . $qualifier . "," . $com . "," . $base_ref->{ $group } . "," . $broad_ref->{ $group };
			}
